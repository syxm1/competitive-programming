...
void solve(int t) {
    ...
    ll bound = a[id-1] + (a[id] - a[id-1]) / 2;
    ll bound2 = a[id-1] + (a[id] - a[id-1]) / 2 + 1;
    ll closest = 1e18;

    for(int i = 0; i < m; i++) {
        int lo = 0, hi = k-1, id1 = -1, id2 = -1;
        while(lo <= hi) {
            int md = (lo + hi) >> 1;
            if(d[i] + f[md] > bound) {
                hi = md-1;
            } else if(d[i] + f[md] == bound) {
                id1 = md;
                break;
            } else {
                lo = md+1;
        ...
        if(abs(closest - bound) > abs(p - bound)) closest = p;
        if(abs(closest - bound) > abs(q - bound)) closest = q;
        ...
        if((a[id] - a[id-1]) & 1) {
            ...
            if(abs(closest - bound2) > abs(p - bound2)) closest = p;
            if(abs(closest - bound2) > abs(q - bound2)) closest = q;
        }
    }
    ...
    mxval = 0;
    for(int i = 1; i < n+1; i++) {
        mxval = max(mxval, a[i] - a[i-1]);
    }

    cout << min(mxval, temp_max) << endl;
