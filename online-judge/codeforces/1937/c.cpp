/*
    ...
    - find the maximum value index (mxp) from the permutation (n-1) by checking p[i] | p[i], 0 <= i <= n-1.
    - store every index with maximum value of p[mxp] | p[j], 0 <= j <= n-1, to an array(id).
    - for every index in id, find the index with minimum value (mnp) by checking p[id[k]] | p[id[k]], 0 <= k <= sz(id).
    - the answer is mxp and mnp.
*/
...
void solve(int tc) {
    ...
    int mxp; // maximum value (n-1)
    if(query(0, 0, 1, 1) == '<') mxp = 1;
    else mxp = 0;

    for(int i = 2; i < n; i++) {
        if(query(mxp, mxp, i, i) == '<') mxp = i;
    }

    vector<int> id; // store index of maximum value p[id] | p[mxp]
    id.pb(0);
    int mxval = 0;

    for(int i = 1; i < n; i++) {
        char c = query(mxval, mxp, i, mxp);
        if(c == '<') {
            mxval = i;
            id.clear();
            id.pb(i);
    ...
    int mnp = id[0]; // minimum p[id];
    for(int i = 0; i < sz(id); i++) {
        char c = query(mnp, mnp, id[i], id[i]);
        if(c == '>') mnp = id[i];
    }
