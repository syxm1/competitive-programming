...
void solve(int tc) {
    ...
    consider cnt_a : count of number <= k that appear only in a
             cnt_b : count of number <= k that appear only in b
             cnt_pair : count of number <= k that appear both in a and b
             
    we can pick both k/2 number from a and b if and only if:
        max(k/2 - cnt_a, 0) + max(k/2 - cnt_b, 0) <= cnt_pair
        
    i.e. if cnt_a less than k/2, we can pick from cnt_pair to make it equals to k/2.
    also applied to cnt_b.
    */
    int n, m, k;
    cin >> n >> m >> k;
    map<int, bool> a, b;
    ...
    int cnt_pair = 0, cnt_a = 0, cnt_b = 0;
    for(auto p : a) {
        if(b[p.fi]) cnt_pair++;
        else cnt_a++;
    }
    ...
    trace(cnt_a, cnt_b, cnt_pair);
    if(max(k/2 - cnt_a, 0)+ max(k/2 - cnt_b,0) <= cnt_pair) {
        cout << "YES" << endl;
    } else {
        cout << "NO" << endl;
    }
