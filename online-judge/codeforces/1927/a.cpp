...
void solve(int tc) {
    ...
    find most left and most right 'B'. the answer is eaual to distance of that two.
    */
    int n;
    cin >> n;
    string s;
    cin >> s;
    int lb = -1, rb = -1;
    for(int i = 0; i < n; i++) if(s[i]=='B') {lb = i; break;}
    for(int i = n-1; i >= 0; i--) if(s[i]=='B') {rb = i; break;}
    if(lb==-1) {
        cout << 0 << endl;
    } else {
        cout << rb - lb + 1 << endl;
