...
void solve(int tc) {
    ...
    consider s[i] sum of a[i] to a[i+k-1].
    one of possible way to build s[i] where s[i]+1 = s[i+1] for odd 'i' and,
                                            s[i] = s[i+1] + 1 for even 'i'.

    thus a[i] = a[i+k] + 1 for odd 'i' and a[i] + 1 = a[i+k] for even 'i'.
    */
    int n, k;
    cin >> n >> k;
