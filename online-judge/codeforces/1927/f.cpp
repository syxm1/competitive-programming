...
use kruskal to find maximum spanning tree. 
find edge with minimum weight that doesn't belong to the tree.
the weight of that edge should be the answer.

backtrack using bfs to find the cycle path (maintaining 'previous' array).
*/
...
void solve(int tc) {
    ...
    tuple<int, int, int> light(inf, inf, inf);
    for(int i=0; i < m; i++) {
        auto [w, u, v] = edges[i];
        if(get(u) != get(v)) {
            merge(u, v);
        } else {
            light = min(light, edges[i]);
        }
    }

    auto [ans, start, end] = light;

    vector<vector<int>> adj(n);
    for(int i = 0; i < m; i++) {
        if(edges[i] == light) continue;
        auto [w, u, v] = edges[i];
        adj[u].pb(v);
        adj[v].pb(u);
    }
