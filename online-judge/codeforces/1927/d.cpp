...
void solve(int tc) {
    ...
    make a prefix array pref.
    for every index i, pref[i] = pref[i-1] + (a[i] != a[i-1]).
    thus, for every query, we need to find closest index 'j' to the right where pref[j] == pref[i] + 1.
    */
    int n;
    cin >> n;
    vector<int> a(n);
    ...
    while(q--) {
        int l , r;
        cin >> l >> r;
        int id = lower_bound(all(pref), pref[l]+1) - pref.begin();
        if(id > r) cout << -1 << ' ' << -1 << endl;
        else cout << l << ' ' << id << endl;
    }
}
