/*
    ...
    binary search on prefix and suffix.
*/
...
void solve(int tc) {
    ...
    vector<ll> pref(n+5, 0), suff(n+5, 0), diff(n+5, 0);
    for(int i = 1; i <= n; i++) pref[i] = pref[i-1] + a[i];
    for(int i = n; i >= 1; i--) suff[i] = suff[i+1] + a[i];
    for(int i = 1; i <= n; i++) diff[i] = diff[i-1] + (a[i-1] != a[i]);
    ...
    for(int i = 2; i <= n; i++) {
        if(a[i-1] > a[i]) {
            ...
            while(l <= r) {
                ...
                if(diff[i-1] - diff[m] >= 1 && pref[i-1] - pref[m-1] > a[i]) {
                    ans[i] = min(ans[i], i-m);
                    l = m + 1;
                } else {
                    r = m - 1;
    ...
    for(int i = n-1; i >= 1; i--) {
        if(a[i+1] > a[i]) {
            ...
            while(l <= r) {
                ...
                if(diff[m] - diff[i+1] >= 1 && suff[i+1] - suff[m+1] > a[i]) {
                    ans[i] = min(ans[i], m-i);
                    r = m - 1;
                } else {
                    l = m + 1;
