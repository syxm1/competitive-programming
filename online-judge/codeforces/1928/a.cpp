...
void solve(int tc) {
    /*
        other rectangles can only be obtained if at least one of a and b are even.
        if one side is even then the other one is odd, and the 2 * odd = even, it can't obtain another rectangle.
    */
    int a, b;
    cin >> a >> b;
    if(a > b) swap(a, b);
