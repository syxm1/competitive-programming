/*
    ...
    only 1x operation needed for any case.
    check where binary search ended, if it equal to where 'x' is, then the answer is 0.
    otherwise swap the position where the binary search ended and 'x'.
*/
...
void solve(int tc) {
    ...
    if(l == posx) cout << 0 << endl;
        ...
        if(p[l] == x) cout << 1 << endl << posx << ' ' << l << endl;
    }
