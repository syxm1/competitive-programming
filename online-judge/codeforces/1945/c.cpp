/*
    ...
    brute-force all possilbe road position.
*/
...
void solve(int tc) {
    ...
    int ans = 0;
    double val = 100000000.00;

    for(int i = 0; i <= n; i++) {
        int num_right = n-i;
        int num_left = i;
        int satisfied_right = pref1[n] - (i > 0 ? pref1[i] : 0);
        int satisfied_left = pref0[i];


        if(satisfied_right * 2 < num_right || satisfied_left * 2 < num_left) continue;

        if(abs(((double)n)/2.00 - i) < val) {
            val = abs(((double)n)/2.00 - i);
            ans = i;
        }
    }
