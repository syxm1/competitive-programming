/*
    ...
    the sum target to make the array equal is the sum of all elements / n.
    for every index 'i' where a[i] < tar, check if there is enough surplus from the previous index to make a[i] = tar.
*/
...
void solve(int tc) {
    ...
    if(n==1) {
        ...
        ll tar = sum/n, savings = 0, ok = 1;
        for(int i = 0; i < n; i++) {
            if(a[i]>=tar) savings += a[i] - tar;
            else {
                if(savings<tar-a[i]) {
                    ok = 0;
                    break;
