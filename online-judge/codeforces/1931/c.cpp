/*
    ...
    consider we do the operation at index (i, j).
    thus condition a[0] = a[1] = ... = a[i-1] and a[j+1] = a[j+2] = ... = a[n-1] must hold.
    i and j can be found by iterating through the array.
*/
...
void solve(int tc) {
    ...
    int kiri = 1, kanan = 1;
    for(int i = 1; i < n; i++) {if(a[i]==a[0]) kiri++; else break;}
    for(int i = n-2; i >= 0; i--) {if(a[i]==a[n-1]) kanan++; else break;}

    // cout << kiri << ' ' << kanan << endl;
    if(a[0]==a[n-1]) {
        cout << max(0, n - (kiri + kanan)) << endl;
    } else if(kiri > kanan) {
        cout << n - kiri << endl;
    } else {
        cout << n - kanan << endl;
