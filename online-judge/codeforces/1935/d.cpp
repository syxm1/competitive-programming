...
void solve(int tc) {
    ...
    for(int i = 0; i < n; i++) {
        ll cnt_d1 = c - s[i] + 1LL;
        ll cnt_d2 = s[i] / 2LL + 1LL;
        if(s[i] & 1) cnt_d2 -= cnt_odd, cnt_odd++;
        else cnt_d2 -= cnt_even, cnt_even++;
        cnt_d1 = max(cnt_d1, 0LL), cnt_d2 = max(cnt_d2, 0LL);
        C += cnt_d1 + cnt_d2 - (s[i]&1 ? (cnt_odd ? 1 : 0) : (cnt_even ? 1 : 0));
        // trace(s[i], cnt_d1, cnt_d2, C);
    }
