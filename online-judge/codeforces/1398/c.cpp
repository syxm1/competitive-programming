...
inline namespace Modular { // don't really know how it worked but somehow useful
...
void solve(int tc) {
    /*
    zero-sum subarray.
        good subarray => a[l] + a[l+1] + ... + a[r-1] + a[r] == r-l+1
    simplify it by subtracting both sides with r-l+1.
        a[l] + a[l+1] + ... + a[r-1] + a[r] - (r-l+1) == r-l+1 - (r-l+1)
        (a[l] - 1) + (a[L+1] - 1) + ... + (a[r-1] - 1) + (a[r] - 1) == 0
    thus we only need to find the number of subarrays with its sum equals to zero.
    */
    int n;
    cin >> n;
