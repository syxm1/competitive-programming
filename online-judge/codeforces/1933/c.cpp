/*
    ...
    brute-forcing all possible pairs of 'x's and 'y's and save all resulting k in a set.
*/
...
vector<vector<ll>> pows(101, vector<ll>(31, 1));

void solve(int tc) {
    cin >> a >> b >> l;
    set<ll> k;
    for(int i = 0; i < 31; i++) {
        if(pows[a][i]>1000000LL) break;
        for(int j = 0; j < 31; j++) {
            if(pows[b][j]>1000000LL) break;
            ll x = (pows[a][i] * pows[b][j]);
            if(l % x == 0) k.insert(l/x);
        }
    }
    cout << k.size() << endl;
}
