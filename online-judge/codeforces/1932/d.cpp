...
void solve(int tc) {
    ...
    for(int i = 0; i < 2 * n; i++) {
        cin >> card[i];
        m[card[i][1]].pb(card[i][0]-'0');
    }
    int unpair = 0, tcount = 0;
    vector<pair<string, string>> ans;
    vector<string> unpaired;
    for(pair<char, vi> p : m) {
        char c = p.fi;
        sort(all(p.se));
        vi a = p.se;
        if(c == trump) {
            ...
            for(int i = 0; i+1 < sz(a); i+=2) {
                str x = to_string(a[i]); x.pb(c);
                str y = to_string(a[i+1]); y.pb(c);
                ans.pb({x, y});
            }
            if(sz(a) % 2) {
                str x = to_string(a[sz(a)-1]); x.pb(c);
                unpaired.pb(x);
            }
        }
    }
    bool impossible = (tcount - unpair < 0 || (tcount - unpair) % 2 == 1);
    if(impossible) {
        ...
        sort(all(m[trump]));
        while(i < sz(unpaired)) {
            str y = to_string(m[trump][i]); y.pb(trump);
            ans.pb({unpaired[i], y}); ++i;
        }
        while(i < sz(m[trump])) {
            str x = to_string(m[trump][i]); x.pb(trump);
            str y = to_string(m[trump][i+1]); y.pb(trump);
            ans.pb({x, y}); i+=2;
        }

        for(pair<str, str> p : ans) {
            cout << p.fi << ' ' << p.se << endl;
        }
    }
}
