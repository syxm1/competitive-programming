/*
    ...
    dp[i] := maximum number of cats fed until time 'i'.

    transition for dp[i], either we feed cat at time i or we don't feed.

    dp[i] = max(dp[i-1], dp[i-'minimum starting time among the cats that stay at time i']).

    maintain the cat that stay at a time using multiset.
*/
...
void solve(int tc) {
    ...
    vector<vector<int>> start(n+5), end(n+5);
    ...
    for(int i = 1; i <= n; i++) {
        ...
        for(int j : start[i]) lf.insert(j);

        if(lf.size()) dp[i] = max(dp[i], (int) lf.size() + dp[*lf.begin()-1]);

        for(int j : end[i]) lf.erase(lf.find(j));
    }
