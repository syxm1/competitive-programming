...
void solve(int tc) {
    /*
    link : https://atcoder.jp/contests/abc338/tasks/abc338_d

    technique used : contribution to the sum & prefix sum with lazy propagation

    possible route length to travel A->B (B > A) is either (I) B - A or (II) N - B + A.
    > if closed route is between A & B, we can only use (II) route.
    > else we use the (I) route.
    
    define c[i] as total range when bridge number 'i' is closed. 
    thus a tour from A->B will contribute (II) if 'i' is between A & B.
    otherwise A->B will contribute (I) to c[i].

    to cut the complexity, use prefix sum with lazy propagation.
    */
    int n, m;
    cin >> n >> m;
