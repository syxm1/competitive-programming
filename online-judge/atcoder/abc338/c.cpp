...
void solve(int tc) {
    ...
    brute-forcing all possible number of cake 'A'.
    */
    int n;
    cin >>n;
    ...
    int mx_a=2e9;
    for(int i = 0; i < n; i++) {
        if(a[i]>0)mx_a=min(mx_a, q[i]/a[i]);
    }
    int ans = 0;
    for(int cnt_a = 0; cnt_a <= mx_a; cnt_a++) {
        int cnt_b = 2e9;
        for(int i = 0; i < n; i++)
            if(b[i]>0) cnt_b = min(cnt_b, max((q[i] - a[i] * cnt_a) / b[i], 0));
