/*
    ...
    -   count the number of divisor by multiplying every ki + 1, 0 <= i <= n-1.
    -   count the sum of divisor using : https://planetmath.org/formulaforsumofdivisors
    -   for each index i, it will contribute pows(xi, sum(ki) * prod_not_i), where:
        -   sum(ki) equals to the sum from 0 to ki.
        -   prod_not_i is the product of kj+1 for all j, j != i.
*/
...
void solve(int tc) {
    ...
    ll num = 1, sum = 1, mul = 1, pref = 1;

    vector<ll> suff(n + 1, 1);

    for(int i = n-1; i >= 0; i--) {
        suff[i] = (suff[i+1] * (number[i].se + 1)) % phi_mod;
    }
    ...
    for(int i = 0; i < n; i++) {
        ...
        // count sum
        ll ss = ((pows(x, k+1) - 1) * pows(x-1, mod-2)) % mod;
        sum = (sum * ss) % mod;

        // count product
        ll jumlah_pangkat = (k * (k + 1) / 2) % phi_mod;
        ll selain = ((suff[i+1] * pref) % phi_mod) % phi_mod;
        ll pangkat = (jumlah_pangkat * selain) % phi_mod;
        mul = (mul * pows(x, pangkat)) % mod;

        // update prefix
        pref = (pref * (number[i].se + 1)) % phi_mod;
    }
