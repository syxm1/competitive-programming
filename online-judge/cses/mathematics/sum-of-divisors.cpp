/*
    ...
    calculate the number of i where n/i = x.
    assume that i is the minimum where n/i = x and j is the maximum.
    then from range i to j, it will contribute ((j - i + 1) * (i + j) / 2) * n/i to the final answer.
*/
...
void solve(int tc) {
    ...
    for(ll l = 1; l <= n;) {
        ...
        ll sum = (((a * b) % mod * c) % mod * 500000004) % mod;
        ans = (ans + sum) % mod;
