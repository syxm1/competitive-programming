/*
    ...
    use inclusion exclusion principles.
    iterate bitmask representation of subset from a.
    for each subset calculate the LCM of it and it will contribute:
    -   n / LCM, if the number of bit is odd.
    -   -(n / LCM), if the number of bit is even.
*/
...
void solve(int tc) {
    ...
    for(ll mask = 1; mask < (1 << k); mask++) {
        ...
        if(clog > logN) continue;
 
        ll cnt = n/mul;
 
        if(!(__builtin_popcountll(mask)&1)) cnt = -cnt;
