/*
    ...
    compute the frequency of every number.
    brute force the answer and iterate the multiple of answer to count occurence of ans in the array.
    if the number of occurence > 1, then there are multiple elements with factor equal to that answer.
*/
...
void solve(int tc) {
    ...
    for(int ans = 1e6; ans >= 2; ans--) {
        ll div = 0;
        for(int j = ans; j <= 1e6; j+=ans) {
            div += cnt[j];
        }
 
        if(div > 1) { // ada lebih dari 1 bilangan yang punya faktor ans
            cout << ans << endl;
            return;
        }
    }
