/*
    ...
    idea: 
    - represent column condition using bitmask
    - for the condition in column 'col', generate the next possible conditions to 'col+1'
*/
...
// generate possible next masks from the current mask
void generate(int row, int mask, int new_mask, vector<int> &nxt_mask) {
    if(row == n+1) {
        nxt_mask.pb(new_mask);
        return;
    }
    ...
    // if in the current mask bit 'row' is activated
    if(m1) {
        generate(row+1, mask, new_mask, nxt_mask);
    }

    // otherwise
    else {
        generate(row+1, mask, new_mask + (1 << row), nxt_mask);
    }

    // if the current and the next mask bit aren't activated
    if(row < n && m1 == 0 && m2 == 0) {
        generate(row+2, mask, new_mask, nxt_mask);
    }
}
...
ll solve(int col, int mask) {
    // reaching column m+1
    if(col == m+1) {
        // only return true if no bit in current mask is activated
        return (mask == 0);
    }
    ...
    // generate possible masks from the current
    vector<int> nxt_mask;
    generate(1, mask, 0, nxt_mask);

    ll ret = 0;
 
    for(int new_mask : nxt_mask)
        ret = (ret + solve(col+1, new_mask)) % mod; // solve for every possible next masks
 
    return dp[col][mask] = ret;
}
 
void Main(int tc) {
    cin >> n >> m;
 
    dp.assign(m+5, vector<ll>((1<<(n+1)), -1));
