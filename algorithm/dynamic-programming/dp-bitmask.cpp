/*
    ...
    idea: 
    - represent people that already grouped using bitmask.
    - dp[mask].fi is the minimum number of group for condition 'mask'
    - dp[mask].se is the capacity used in the last group based on dp[mask].fi
*/
...
void solve(int tc) {
    ...
    // dp[mask].fi = minimum number of rides with condition mask
    // dp[mask].se = last rides weight
    ...
    for(ll mask = 1; mask < (1<<n); mask++) {
        for(int i = 0; i < n; i++) {
            if(mask & (1 << i)) {
                ...
                if(dp[prev].se + w[i] <= x) {
                    dp[mask] = min(dp[mask], {dp[prev].fi, dp[prev].se + w[i]});
                }
 
                else {
                    dp[mask] = min(dp[mask], {dp[prev].fi+1, w[i]});
                }
            }
        }
    }
